...
def _assert_equal(expected, actual, message_prefix=""):
    assert expected == actual, f"{message_prefix}Expected: '{expected}', Actual: '{actual.strip()}'"

def _assert_not_equal(expected, actual, message_prefix=""):
    assert expected != actual, f"{message_prefix}Expected: '{expected}', Actual: '{actual.strip()}'"

def validate_text_input(driver, locator_type, locator_value, label_expected_text, value_to_type, timeout=10):
    ...
    label_elem = input_elem.find_element(
        By.XPATH, "./parent::td/preceding-sibling::td"
    )
    ...
    _assert_equal(input_elem.get_attribute("value"), value_to_type, "Value not properly set")

def validate_pre_filled_text_input(driver, locator_type, locator_value, pre_filled_text, label_expected_text, value_to_type, timeout=10):
    # Wait for and locate input
    input_elem = WebDriverWait(driver, timeout).until(
        EC.visibility_of_element_located((locator_type, locator_value))
    )
    ...
    _assert_equal(input_elem.get_attribute("value"), pre_filled_text, "Value not properly set")

    # Append the text
    print(f"[pre_filled_input] Appending '{value_to_type}' to pre-filled '{pre_filled_text}' in '{locator_value}'")
...
def validate_placeholder_field_text_input(driver, locator_type, locator_value, placeholder_text, label_expected_text, value_to_type, timeout=10):
    # Wait for and locate input
    input_elem = WebDriverWait(driver, timeout).until(
        EC.visibility_of_element_located((locator_type, locator_value))
    )
    ...
    # Enter the text
    print(f"[placeholder_input] Typing '{value_to_type}' into placeholder '{placeholder_text}' for '{locator_value}'")
    input_elem.send_keys(value_to_type)

    # Validate value was entered
    _assert_equal(input_elem.get_attribute("value"), value_to_type, "Value not properly set")

def validate_textarea_input(driver, locator_type, locator_value, label_expected_text, values_to_type, timeout=10):
...
def validate_SVG(driver, locator_type, locator_value, label_expected_text, stroke_color, fill_color, timeout=10):
    ...
    _assert_not_equal(initial_opacity, updated_opacity, f"Opacity did not change for '{locator_value}', indicating SVG rect animation did not run")

def validate_dropdown(driver, menu_locator_type, menu_locator_value, contents_locator_type, contents_locator_value, expected_options, selected_option, timeout=10):
    ...
    WebDriverWait(driver, timeout).until(
        EC.visibility_of_element_located((contents_locator_type, contents_locator_value))
    )

    # Validate options
    dropdown_contents_elem = driver.find_element(contents_locator_type, contents_locator_value)
    dropdown_options = [option.text.strip() for option in dropdown_contents_elem.find_elements(By.TAG_NAME, "a")]
    _assert_equal(expected_options, dropdown_options, f"Dropdown options mismatch for '{menu_locator_value}'")

    # Select the option
    print(f"[dropdown] Selecting '{selected_option}' in '{menu_locator_value}'")
    option_elem = dropdown_contents_elem.find_element(By.XPATH, f".//a[normalize-space(text())='{selected_option}']")
    ActionChains(driver).move_to_element(option_elem).click().perform()

    # Validate header changes after making dropdown selection
    header_elem = driver.find_element(By.XPATH, f".//h3[text()='{selected_option} Selected']")
    _assert_equal(header_elem.text.strip(), f"{selected_option} Selected", f"Header text mismatch after selecting '{selected_option}' in '{menu_locator_value}'")
