...
def test_page_title_is_correct(driver: WebDriver) -> None:
    """
    Ensure that the form validation page loads correctly and the
    expected title is present. Skipping placeholder/default field 
    text checks — low-impact, brittle, and not core to test value.
    """
...
def test_valid_form_submission_redirects(driver: WebDriver) -> None:
    ...
    page.assert_startswith(expected_msg, 
                           actual_msg, 
...
def test_blank_form_submission_shows_errors(driver: WebDriver) -> None:
    """
    Click Register with all fields blank and validate error messages
    for each.
    """
    page = FormValidationPage(driver)
    page.open()
    ...
    page.assert_equal(expected_name_error_msg, 
                      actual_name_error_msg, 
                      "Name field error message mismatch: "
    )
    page.assert_equal(expected_number_error_msg, 
                      actual_phone_error_msg, 
                      "Phone field error message mismatch: "
    )
    page.assert_equal(expected_date_error_msg, 
                      actual_date_error_msg, 
                      "Date field error message mismatch: "
    )
    page.assert_equal(expected_payment_method_error_msg, 
                      actual_payment_method_error_msg, 
                      "Payment method field error message mismatch: "
    )

@pytest.mark.form_validation
def test_field_validation_feedback(driver: WebDriver) -> None:
    """
    Trigger validation errors, then fill out one field at a time with
    valid input and verify they show success indicators.
    
    - Name field should display positive acknowledgment text.
    - Contact number, PickUp Date, and Payment Method should display
      green checkmarks with no accompanying text.
    """
    page = FormValidationPage(driver)
    page.open()
    ...
    page.assert_equal(expected_name_success_msg, 
                      actual_name_success_msg, 
                      "Name field success message mismatch: "
    )
    page.assert_equal(True, 
                      page.get_phone_success_indicator(), 
                      "Phone field should suppress error message: "
    )
    page.assert_equal(True, 
                      page.get_date_success_indicator(), 
                      "Date field should suppress error message: "
    )
    page.assert_equal(True, 
                      page.get_payment_success_indicator(), 
                      "Payment method field should suppress error message: "
    )